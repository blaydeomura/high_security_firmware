...
use p256::ecdsa::{signature::Verifier, signature::Signer, SigningKey};
...
// Helper functions for non-quantum operations, assume these exist or implement accordingly
fn rsa_sign(buffer: &[u8], private_key_bytes: &[u8]) -> io::Result<Vec<u8>> {
    let private_key = RsaPrivateKey::from_pkcs8(private_key_bytes)
...
fn ecdsa_sign(buffer: &[u8], private_key_bytes: &[u8]) -> io::Result<Vec<u8>> {
    let signing_key = SigningKey::from_bytes(private_key_bytes)
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, e.to_string()))?;
    
    let signature = signing_key.sign(buffer);
    Ok(signature.as_ref().to_vec())
}

fn eddsa_sign(buffer: &[u8], private_key_bytes: &[u8]) -> io::Result<Vec<u8>> {
...
pub fn sign(name: &str, file_path: &str, wallet: &Wallet) -> io::Result<()> {
    ...
    let hash_result_vec = get_hash(persona.get_cs_id(), &buffer)?;

    let signature = match get_sig_algorithm(persona.get_cs_id())? {
        AlgorithmType::QuantumSafe(algo) => {
            let sig_algo = OqsSig::new(algo).expect("Failed to create OQS Sig object");
            sig_algo.sign(&hash_result_vec, &persona.get_sk().expect("Quantum SK not found"))
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Signing failed: {}", e)))?
        },
        AlgorithmType::NonQuantumSafe(algo) => {
            match algo {
                NonQuantumAlgorithm::RSA2048 => rsa_sign(&hash_result_vec, &persona.get_non_quantum_sk().expect("Non-quantum SK not found")),
                NonQuantumAlgorithm::ECDSAP256 => ecdsa_sign(&hash_result_vec, &persona.get_non_quantum_sk().expect("Non-quantum SK not found")),
                NonQuantumAlgorithm::EdDSAEd25519 => eddsa_sign(&hash_result_vec, &persona.get_non_quantum_sk().expect("Non-quantum SK not found")),
            }?
        },
    };
