...
impl CipherSuite for RsaSha256 {
    ...
    fn peer_verify(&self, signed_data: SignedData, pk: Vec<u8>, cs_id: usize) -> io::Result<()> {
        ...
        signed_data.get_header().verify_hash(&hash_based_on_cs_id(cs_id, signed_data.get_contents())?);
        ...
        // verifying_key.verify(&computed_hash, &signature).expect("Verification failed at verifying key\n");
        verifying_key.verify(&hashed_header, &signature).expect("Verification failed at verifying key\n");
